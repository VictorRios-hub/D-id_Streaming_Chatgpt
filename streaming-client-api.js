//CMD node app.js on http://localhost:3000/
...
async function fetchOpenAIResponse(userMessage) {
  const response = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      // Temperature is used to determine the originality of the response : 0 -> sure & 1 -> original
      temperature: 0.5,
      // Max tokens influences the number of word the model will use. Increase to have longuer responses but it will take more time to answer.
      max_tokens: 100
    }),
  });
...
function onVideoStatusChange(videoIsPlaying, stream) {
  ...
  if (videoIsPlaying) {
    ...
    // Check if streaming have already started
